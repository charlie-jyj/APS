...
seoul2_10_wait = 0  # 전역변수


def Me(opp, turn, opp_prev, opp_last_pattern):
    global seoul2_10_wait

    if turn == 0 :  # 나는 시작할 때 관대하다
        return 0
    else:
        # 상대방에게 이전 게임 기록이 있을 경우, 의미가 있는 count
        deer_cnt = opp_last_pattern.count(0)
        rabbit_cnt = opp_last_pattern.count(1)
        snake_cnt = opp_last_pattern.count(2)

        if snake_cnt > deer_cnt + rabbit_cnt:  # 악인은 응징
            return 2
        else:
            if opp_prev == 2:  # 나를 배신했다면 2번까지는 봐준다
                seoul2_10_wait += 1
                if seoul2_10_wait > 2 :
                    seoul2_10_wait = 0
                    return 2

            return 0  # 기본적으로는 관대하다.
